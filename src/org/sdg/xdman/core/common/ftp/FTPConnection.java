...
public class FTPConnection extends Connection {
	...
	public FTPConnection(String url, String fileName, long startOff,
	...
	public FTPConnection(State state, int timeout, ConnectionManager mgr,
	...
	public boolean connect() {
		...
		while (true) {
			...
			try {
				...
				if (config.useFtpProxy) {
					String proxyHost = config.ftpProxyHost;
					int proxyPort = config.ftpProxyPort;
					String proxyUser = config.ftpUser;
					String proxyPass = config.ftpPass;
					...
					if (proxyAuth) {
						msg("Authenticating with: " + proxyUser + ":"
								+ proxyPass);
				...
				chkPause();
				if (credential == null) {
					credential = Authenticator.getInstance()
							.getCredential(host);
				}
				message = "Logging in...";
				mgr.updated();
				...
				if (!FTPReply.isPositiveCompletion(reply)) {
					throw new Error(
							"Could not switch to the sprecified directory.");
				}
				if (length > 0) {
					...
					if (!FTPReply.isPositiveCompletion(reply)) {
						throw new Error(
								"File information could not be retrived.");
					}
